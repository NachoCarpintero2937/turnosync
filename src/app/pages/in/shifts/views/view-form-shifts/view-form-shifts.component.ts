...
export class ViewFormShiftsComponent implements OnInit, OnChanges {
  ...
  initComponent() {
    ...
    this.form.get('status')?.valueChanges.subscribe((status: number | null) => {
      this.showPaymentSelector = status === 1;
      if (status !== 1) {
  ...
  selectPaymentMethod(method: 'efectivo' | 'transferencia') {
    this.form.get('payment_method')?.setValue(method as any);
  }
  ...
  setValueFromShift() {
    ...
    this.form
      .get('hour')
      ?.setValue(this.DatePipe.transform(this.shift?.date_shift, 'HH:mm'));
    this.form.get('description')?.setValue(this.shift?.description);
    // this.form.get('status').setValue();

    let p = this.shift?.price ? this.shift.price.toString() : '';
    if (p.endsWith('.00')) p = p.replace('.00', '');
    this.form.get('price')?.setValue(p);
  ...
  setValueForm() {
    ...
    // Si form.hour tiene algo, lo usamos, si no por defecto la hora actuall
    let hourVal = this.form.get('hour')?.value;
    if (!hourVal) {
      hourVal = this.DatePipe.transform(date, 'HH:mm');
      this.form.get('hour')?.setValue(hourVal, { emitEvent: false });
    }

    if (hourVal) {
      const [hours, minutes] = hourVal.split(':');
    ...
    this.form
      .get('user_id')
      ?.setValue(this.LoginService.getDataUser().data?.id);
    if (this.client) {
  ...
  searchMat() {
    ...
    this.searchCtrlClient.valueChanges
      .pipe(
        startWith(''),
    ...
    this.searchCtrlService.valueChanges
      .pipe(
        startWith(''),
    ...
    this.searchCtrlUsers.valueChanges
      .pipe(
        startWith(''),
    ...
    this.form.get('hour')?.valueChanges.subscribe((hour: any) => {
      ...
      if (date && hour) {
        ...
        this.dateTransform = this.DatePipe.transform(
          fullDate,
          'yyyy/MM/dd HH:mm:ss',
        );
      }
  ...
  onSelect(type: string) {
    if (type == 'service') {
      ...
      this.filteredService = this.services;
      const price = this.services.find(
        (service: any) => service?.id === this.form.get('service_id')?.value,
      );
  ...
  submit() {
    ...
    const data = this.ShiftService.mapToShift(raw, this.dateTransform);
    this.ShiftService.setShift(data)
      .then((data) => {
        this.loading.emit(false);
